...
#include "USB_Stylocard/USB_Stylocard.h"
...
int main(void) {
  ...
  //setup timer 0 for making 1khz interrupst
  TCCR0A = 0x02;
  TCCR0B = 0x03;
  OCR0A = 250;
  TIMSK |= (1 << OCIE0A);
  //use this for waiting for the computer to recognise the device
  midi.delay(1500);
  TCCR1 = 0b10001011;
  OCR1A = 156;
  TIMSK |= (1 << OCIE1A);
      ...
      if ((millis - time) > 1000) {
        ...
        for (int a = 0; a != 20; a++) {
          ...
          if (value < 900) {
            ...
            voltage[a] = ADC;
            //write the low ADC uint8_t in the first EEPROM address
            write_eeprom(a << 1, ADCL);
            //write the higher ADC uint8_t int the next address
...
ISR(TIMER0_COMPA_vect) {
  // act as the millis funtion of the arduino IDE
  millis++;
}
